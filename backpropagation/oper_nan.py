import math
def back_mul(a, b, rlt_posi):
    "a * x = b."
    if a == 0 and b != a:
        return b / math.sqrt(1 + a * a)
    else:
        return b / a if b != a else 1

def back_sub(a, b, rlt_posi):
    "a - x = b or x - a = b."
    if rlt_posi == 0:#x-a
        return a + b
    else:#a-x
        return a - b

def back_truediv(a, b, rlt_posi):
    "a / x = b or x / a = b."
    if rlt_posi == 0:#x/a
        return a * b
    else:#a/x

        if b == 0 and b != a:
            return a / math.sqrt(1 + b * b)
        else:
            return a / b if b != a else 1

def back_add(a, b, rlt_posi):
    "a + x = b"
    return b - a

def back_neg(a, rlt_posi):
    "x=-a"
    return -a

'''return NAN if a out of [-1, 1] '''
def back_sin(a, rlt_posi):
    "x=sin a"
    if -1 <= a <= 1:
        return math.asin(a)
    else:
        return float('nan')

def back_tan(a, rlt_posi):
    "x=sin a"
    if -1 <= a <= 1:
        return math.atan(a)
    else:
        return float('nan')

def back_cos(a, rlt_posi):
    "x=cos a"
    if -1 <= a <= 1:
        return math.acos(a)
    else:
        return float('nan')

def back_exp(a, rlt_posi):
    "x=exp a"
    if a>0:
        return math.log(a)
    else:
        return float('nan')
def back_log(a, rlt_posi):
    "x=log a"
    return math.exp(a)
def back_asin(a, rlt_posi):
    "x=asin a"
    return math.sin(a)
def back_acos(a, rlt_posi):
    "x=acos a"
    return math.cos(a)
def back_atan(a, rlt_posi):
    "x=atan a"
    return math.tan(a)

oper_set = {'mul': back_mul, 'div': back_truediv, 'sub': back_sub, 'add': back_add, 'neg': back_neg, 'sin': back_sin, 'cos': back_cos, 'tan': back_tan, 'exp': back_exp, 'ln': back_log, 'asin': back_asin, 'acos': back_acos, 'atan': back_atan}