...
class Generator(object):
    ...
    # give me a fuction to turn the infix to prefix:
    ...
    @classmethod
    def to_prefix_corrected(cls, expr):
        # Base case: if the expression is an atom (like a variable, a number, or 'constant'), return it as is
        if expr.is_Atom:
            return str(expr)

        # Handle the special case of division, represented as a power of -1
        if isinstance(expr, sp.Pow) and expr.args[1] == -1:
            return f"/ {cls.to_prefix_corrected(expr.args[0])}"
        ...
        operands = [cls.to_prefix_corrected(arg) for arg in expr.args]

        # Special handling for division within multiplication
        if op_symbol == '*' and any(isinstance(arg, sp.Pow) and arg.args[1] == -1 for arg in expr.args):
            div_operand = next(arg.args[0] for arg in expr.args if isinstance(arg, sp.Pow) and arg.args[1] == -1)
            non_div_operands = [arg for arg in expr.args if not (isinstance(arg, sp.Pow) and arg.args[1] == -1)]
            return f"/ {cls.to_prefix_corrected(sp.Mul(*non_div_operands))} {cls.to_prefix_corrected(div_operand)}"

        # Join the operation symbol with operands
        return f"{op_symbol} " + " ".join(operands)

    @classmethod
    def reverse_add_sub(cls, expr):
    ...
    @classmethod
    def infix_to_prefix(cls, expression):
        ...
        def get_precedence(operator):
            precedence = {'+': 1, '-': 1, '*': 2, '/': 2, 'sin': 3, 'cos': 3, 'tan': 3, 'exp': 3, 'ln': 3, 'asin': 3,
                          'acos': 3, 'atan': 3}
    ...
    @classmethod
    def split_expr(cls, prefix_expr):
        """ Enhanced approach to tokenize the prefix expression with specific handling for variables x_0 to x_9. """
        # Define patterns for operators, functions, constants, and variables
        operators_functions = r'(/|\+|-|\*|\*\*|sin|cos|tan|asin|acos|atan|exp|log|constant)'
        variable_pattern = r'(\bx_[0-9])'
        # constant_pattern = r'(\bc_[0-9])'

        tokens = []
        current_token = ''
        i = 0

        while i < len(prefix_expr):
            char = prefix_expr[i]

            if re.match(operators_functions, prefix_expr[i:]):
                if current_token:
                    tokens.append(current_token)
                    current_token = ''

                match = re.match(operators_functions, prefix_expr[i:]).group()
                tokens.append(match)
                i += len(match)
            elif re.match(variable_pattern, prefix_expr[i:]):
                if current_token:
                    tokens.append(current_token)
                    current_token = ''

                match = re.match(variable_pattern, prefix_expr[i:]).group()
                tokens.append(match)
                i += len(match)

            # elif re.match(constant_pattern, prefix_expr[i:]):
            #     if current_token:
            #         tokens.append(current_token)
            #         current_token = ''
            #
            #     match = re.match(constant_pattern, prefix_expr[i:]).group()
            #     tokens.append(match)
            #     i += len(match)
    ...
    # def split_expr(cls, prefix_expr):
    #     """ Enhanced approach to tokenize the prefix expression to include mathematical functions. """
    #     # List of known operators and functions
    #     operators_functions = {'/', '+', '-', '*', 'sin', 'cos', 'tan', 'asin', 'acos', 'atan', 'exp', 'log'}
    #     variable_constant_pattern = r'(constant|x_\d+)'
    #     function_pattern = r'(sin|cos|tan|asin|acos|atan|exp|log)'
    #
    #     tokens = []
    #     current_token = ''
    #     i = 0
    ...
    #         char = prefix_expr[i]
    #
    #         # Check for variables, constants, or functions
    #         if re.match(variable_constant_pattern, prefix_expr[i:]):
    #             match = re.match(variable_constant_pattern, prefix_expr[i:]).group()
    #             tokens.append(match)
    #             i += len(match)
    #         elif re.match(function_pattern, prefix_expr[i:]):
    #             match = re.match(function_pattern, prefix_expr[i:]).group()
    #             tokens.append(match)
    #             i += len(match)
    #         elif char in operators_functions or (current_token + char in operators_functions):
    #             current_token += char
    #             # If next char does not extend the operator, add it to tokens
    #             if i + 1 >= len(prefix_expr) or not current_token + prefix_expr[i + 1] in operators_functions:
    #                 tokens.append(current_token)
    #                 current_token = ''
    #             i += 1
    #         else:
    ...
    # def to_prefix_special_final(cls, expr):
    #     # Base case: if the expression is an atom (like a variable or a number), return it as is
    #     if expr.is_Atom:
    #         return str(expr)
    #
    #     # Handle the special case of division, represented as a power of -1
    #     if isinstance(expr, sp.Pow) and expr.args[1] == -1:
    #         return f"/ {cls.to_prefix_special_final(expr.args[0])}"
    #
    #     # Get the operation symbol
    #     op = expr.func.__name__
    #     if op == 'Mul':
    ...
    #
    #     # Recursively convert each argument
    #     operands = [cls.to_prefix_special_final(arg) for arg in expr.args]
    #
    #     # Correctly group operands for addition and multiplication
    #     if op_symbol in ['+', '*']:
    #         # If there are more than two operands, group them correctly
    #         while len(operands) > 2:
    #             first, second, *remaining = operands
    #             operands = [f"{op_symbol} {first} {second}"] + remaining
    #         return f"{op_symbol} " + " ".join(operands)
    #     else:
    #         return f"{op_symbol} " + " ".join(operands)


    # def to_prefix_special(cls, expr):
    #     # Handle the special case of division
    #     if isinstance(expr, sp.Pow) and expr.args[1] == -1:
    #         return f"/ {cls.to_prefix_special(expr.args[0])}"
    #     elif isinstance(expr, sp.Mul) and any(isinstance(arg, sp.Pow) and arg.args[1] == -1 for arg in expr.args):
    #         # Identify the division part and the other part in multiplication
    #         div_operand = next(arg.args[0] for arg in expr.args if isinstance(arg, sp.Pow) and arg.args[1] == -1)
    #         other_operand = next(arg for arg in expr.args if not (isinstance(arg, sp.Pow) and arg.args[1] == -1))
    #         return f"/ {cls.to_prefix_special(other_operand)} {cls.to_prefix_special(div_operand)}"
    #
    #     # Base case: if the expression is an atom (like a variable or a number), return it as is
    #     if expr.is_Atom:
    #         return str(expr)
    ...
    #         op_symbol = '**'
    #     else:
    #         # Use the function name directly for other operations (for simplicity)
    #         op_symbol = op
    #
    #     # Recursively convert each argument and join them with the operation symbol
    #     operands = [cls.to_prefix_special(arg) for arg in expr.args]
    #     return f"{op_symbol} " + " ".join(operands)

    @classmethod
    def expand_power(cls, expression):
        return str(sp.expand(expression))

    @classmethod
    def expand_repeat(cls, expression):
        def expand(match):
            base = match.group(1)
